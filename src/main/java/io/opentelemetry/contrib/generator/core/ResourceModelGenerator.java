...
public class ResourceModelGenerator {
    ...
    private void mapChildResources() {
        ...
        for (ResourceDefinition parentType: allResources.values().stream()
            ...
            for (Map.Entry<String, String> eachChildTypeExpr: parentType.getChildrenDistribution().entrySet()) {
                ...
                //Map any remaining child resources to the last parent resource
                if (nextChildIndex < childrenSize) {
    ...
    private void executeAttributeOperations() {
        Set<String> resourcesWithOperations = allResources.values().stream()
        ...
        List<Set<String>> resourceLevels = resourcesWithOperations.isEmpty() ? new ArrayList<>() : getResourceLevels();
        resourceLevels.forEach(resources -> resources.stream()
                .filter(resourcesWithOperations::contains)
    ...
    private int getResourceLevel(String resourceType) {
        if (CollectionUtils.emptyIfNull(typeMappings.get(resourceType).getParentTypes()).isEmpty()) {
            return 0;
        }
        int level = typeMappings.get(resourceType).getParentTypes().size();
        for (String eachParent: typeMappings.get(resourceType).getParentTypes()) {
